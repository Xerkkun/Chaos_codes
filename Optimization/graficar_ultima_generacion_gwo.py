...
def load_sheet(sheet_number):
    # Cargar la hoja especificada
    sheet_name = excel_data.sheet_names[sheet_number]
    df = pd.read_excel(file_path, sheet_name=sheet_name)

    # Extraer las columnas relevantes
    a = df.iloc[:, 1].values  # Columna H
    b = df.iloc[:, 2].values    # Columna I
    c = df.iloc[:, 3].values   # Columna J
    DKY = df.iloc[:, 4].values   # Columna K
...
file_path = '/home/xerk/Escritorio/Códigos para artículo/Artículo PSO ED GW/Analisis de Convergencia Chen.xlsx'
excel_data = pd.ExcelFile(file_path)
# Leer los parámetros del archivo

for semilla in range(0,10):
    # Ejemplo de uso con la hoja 0
    sheet_number = semilla
    a, b, c, DKY = load_sheet(sheet_number)
    ...
    for j in range(0, 20):
        ...
        # Resolver el sistema de Lorenz usando Euler hacia adelante
        for i in range(num_steps):
            sol[i + 1] = euler_forward(sol[i], t_step, a[j], b[j], c[j])
        ...
        t2 = np.linspace(t_step * transient, t_step *
                        num_steps, num_steps + 1 - transient)
        dt2 = t2[1] - t2[0]
        ...
        file_name = "GWO_Chen_semilla" + str(semilla) + "_" + str(j) + ".pdf"
        print(j)
