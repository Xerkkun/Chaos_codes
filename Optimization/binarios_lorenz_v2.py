...
def runge_kutta4(X, h, a, b, c):
    k1 = lorenz_equations(X,  a, b, c)
    k2 = lorenz_equations(X + h*0.5*np.array(k1),  a, b, c)
    k3 = lorenz_equations(X + h*0.5*np.array(k2),  a, b, c)
    k4 = lorenz_equations(X + h*np.array(k3),  a, b, c)
...
# pos_pro: método para posprocesamiento (xor,xor_shift)
...
aa, bb, cc = 10, 28, 8./3.
# aa, bb, cc= 5.301300510626442808, 135.2804249862039967, 0.7840922407503264635
# aa, bb, cc = 0.458987,	0.476328,	2.51389

x_eq, y_eq, z_eq, t_step, tt = valores_propios_lorenz(aa, bb, cc)

nn, ss, v, b, nstp, pos_pro = input('Parámetros de entrada: ').split()
...
arch = open("lorenz_"+ b + "_" + pos_pro + "_" + v +"original.rnd", "wb")
r, i = -1, -1
