...
# pos_pro: método para posprocesamiento (none,xor,xor_shift,gray)

imagen = 'lena'
image1 = cv2.imread(imagen+'.jpg')

alto,ancho,color = image1.shape
nn,ss,tt,met,v,b,nstp,pos_pro = color*ancho*alto,1,5000,'RK4','z','mod255',1,'xor_shift'
n,s,t,nstep = int(float(nn)),int(ss),int(tt),int(nstp)

xo,yo,wo,zo = -0.1,-0.1,0.,0.

print("Nombre de la imagen: ", imagen + '.jpg')
...
hh = (0.001,0.01,0.001,0.005,0.005,0.005) #Ancho de paso para cada método
r,i = -1,-1
binp = ''
pos_bin = ''
...
for element in itertools.product(dim_i,dim_j,dim_k):
    ...
    if len(binp)==8 and pos_pro == "xor_shift":
        for j in range(0,8):
    ...
    elif len(binp)==8 and pos_pro == "none":
        pos_bin = binp
        encryp[element] = int(pos_bin,2)
        pos_bin = ''
        binp = ''

cv2.imwrite('encryp.jpg',encryp)
encryp2 = cv2.imread("encryp.jpg")
xor1 = cv2.bitwise_xor(image1, encryp2)
cv2.imwrite(imagen + ' ' + pos_pro + '.jpg',xor1)
rec1 = cv2.bitwise_xor(xor1, encryp2)
cv2.imwrite(imagen + ' ' + 'rec ' + pos_pro + '.jpg',rec1)
