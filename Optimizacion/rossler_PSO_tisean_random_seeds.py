...
# Condición para descartar evoluciones temporales periódicas
    ...
    if (sumax > 20 and sumaz < 100 and num_peaksx > 70):
        ...
        output_file = "Rossler_pso.dat"
        np.savetxt(output_file, sol, delimiter="  ")

        # print("Datos guardados en", output_file)

        lyap_specPATH = 'lyap_spec Rossler_pso.dat -x' + \
            str(transient) + ' -c1,2,3 -m3,1 -k5 -osalida_pso_rossler.lyaps'
        ...
        with open("salida_pso_rossler.lyaps", "r") as file:
            file.seek(0, 2)
...
for run in range(0, 10):
    f = open("progress_pso_rossler_tisean" + str(run) + ".txt", 'w')
    original = sys.stdout
    ...
    output_file = "last_gen_pso_rossler_tisean" + str(run) + ".dat"
