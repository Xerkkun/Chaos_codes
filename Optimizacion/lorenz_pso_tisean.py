...
# Condición para descartar evoluciones temporales periódicas       
    ...
    if(sumax > 150 and num_peaksx > 200):
        ...
        #for i in parameters:
        #lyap_specPATH = Path('F:/Fer/TISEAN_3-windows/Tisean_3/bin/lyap_spec') #Cambiar esta ruta
        #parametersPATH = ' Lorenz_pso.dat -x' + str(transient) + ' -c1,2,3 -m3,1 -k' +str(i) + ' -n' +str(n) + ' -osalida_pso.lyaps'
        #check_output(str(lyap_specPATH)+parametersPATH, shell=True)
        
        lyap_specPATH = 'lyap_spec Lorenz_pso.dat -x' + str(transient) + ' -c1,2,3 -m3,1 -k' +str(i) + ' -osalida_pso.lyaps'
...
#        LE_mean = LE_sum/len(parameters)
        file2 = open("iteraciones_pso.dat", "a")
...
np.savetxt('objectives_history_pso.txt', objectives_history)
