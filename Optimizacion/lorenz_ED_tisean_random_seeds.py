...
# Condición para descartar evoluciones temporales periódicas       
    ...
    if(sumax > 150 and num_peaksx > 200):
        ...
        #check_output(str(lyap_specPATH)+parametersPATH, shell=True)
            ...
            lyap_specPATH = 'lyap_spec Lorenz_de.dat -x' + str(transient) + ' -c1,2,3 -m3,1 -k' +str(i) + ' -osalida_de.lyaps'
            ...
            with open("salida_de.lyaps", "r") as file:
                file.seek(0,2)
...
for run in range(0,10):
    f = open("progress_de_" + str(run) + ".txt", 'w')
    original = sys.stdout
    sys.stdout = Tee(sys.stdout, f)
    ...
    output_file = "last_gen_de_" + str(run) + ".dat"
