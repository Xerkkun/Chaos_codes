...
# Condición para descartar evoluciones temporales periódicas
    ...
    if (sumax > 20 and sumaz < 100):
        ...
        output_file = "chen_de.dat"
        np.savetxt(output_file, sol, delimiter="  ")

        # print("Datos guardados en", output_file)

        lyap_specPATH = 'lyap_spec chen_de.dat -x' + \
            str(transient) + ' -c1,2,3 -m3,1 -k100 -r0.01 -osalida_de_chen.lyaps'
            ...
            with open(archivo, "r") as file:
                for linea in file:
                    ...
                    if len(partes) >= 3:  # Asegurar que la línea tiene al menos 3 partes
                        valor = partes[2]  # Obtener el valor de la tercera columna
...
for run in range(0, 1):
    f = open("progress_de_chen_tisean" + str(run) + ".txt", 'w')
    original = sys.stdout
    ...
    output_file = "last_gen_de_chen_tisean" + str(run) + ".dat"
