...
if n < m:   # Si la cantidad de pasos totales es menor a la memoria disponible
    nu,mm = 1,n
else:       # Si la cantidad de pasos totales es mayor a la memoria disponible (FPGA)
    nu,mm = m,m
...
#===============================================================
    ...
    if nu == 1:
        ...
        # Desplazar todos los valores del vecto x una posicion hacia arriba y dejar vacio el último elemento del vector
            ...
            new_x = (omega_coef[nu-1]*x[0] - sum + np.power(h,mu)*math.gamma(2-mu)*fx(x[i-1],a,b,fxTau))/omega_coef[0]
